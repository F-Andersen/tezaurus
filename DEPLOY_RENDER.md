...
### Локально vs Render (чому поради про `.env` з інтернету не завжди підходять)

| Де | Звідки береться `DATABASE_URL` |
|----|--------------------------------|
| **Локально** | Файл **`apps/api/.env`** (не в git), рядок `DATABASE_URL="postgresql://..."`. Команди Prisma запускайте з **`apps/api`** (`cd apps/api` → `npx prisma migrate dev` тощо). Ім’я файлу саме `.env`, без зайвих пробілів у рядку. |
| **Render (Docker)** | Файл `.env` у контейнер **не потрапляє** з репозиторію. Prisma CLI на старті читає **змінні середовища процесу** — їх задає **Render → Environment** (аналог `docker run -e DATABASE_URL=...`). Тут проблема «немає `.env`» вирішується не файлом, а змінною в Dashboard. |

У цьому репозиторії **Prisma 5** — схема з `url = env("DATABASE_URL")` у `schema.prisma` коректна; нюанси **Prisma 7** з `prisma.config.ts` до вашого проєкту не застосовуються. Змінні для **Next.js** (`apps/web/.env.local`) живуть окремо і не підставляються в API-контейнер на Render автоматично.
